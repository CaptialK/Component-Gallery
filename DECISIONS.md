...
### 2026-05-05 — THE STANDARD (medical version) applied to the medical section

- **Problem.** The 2026-05-05 SaaS-only STANDARD pass left the medical section behind, with the explicit (then-defensible) rationale that *clinical SaaS reads more honestly as a frozen specimen because it isn't a real EHR* — interactivity there crosses into cosplay. That holding pattern was meant to be revisited; it wasn't the destination. Read against the bar above it, the medical plates were thinner than the SaaS section: no loading skeletons, no empty-vs-not-collected discipline, abnormal-by-color-only in places, missing units on values, missing timestamps on clinical data, no pending/partial/amended/corrected states for labs and meds, no per-metric stale persimmon, no responsive collapse, allergies that conflated `null` (not documented — a safety hazard) with `[]` (NKDA verified). For a portfolio reader who is a clinician — exactly the audience the medical section earns its keep with — these gaps read louder than they would on SaaS, because medical UX has a different physics: ambiguity is a defect, stale data is dangerous, empty is not normal, color alone is not signal. Specimen voice was a defensible holding pattern. It was not the bar.
- **Options.**
  - (a) Keep the specimen framing forever. The pure-folio voice survives; the medical section reads as printed plates rather than as catalogued working software. Cleanest aesthetic story; loses the audience that would actually evaluate clinical UX.
  - (b) Apply the SaaS THE STANDARD verbatim. Loading + empty + error + responsive + a11y + edge cases — same checklist, same primitives. Gets the medical section to product-grade by SaaS rules, but SaaS rules don't encode the clinical bar (units, abnormal-flag triad, timestamps on every value, freshness thresholds, real LOINC/RxNorm, allergies discrimination).
  - (c) Codify a stricter MEDICAL STANDARD that subsumes THE STANDARD and adds the clinical-safety axis on top. Build clinical primitives that enforce the bar by construction — `ClinicalValue` makes value+unit+flag+timestamp atomic so a builder *can't* ship a value without a timestamp; `AbnormalFlag` makes the icon+letter+color triad the only abnormal vocabulary so color-alone-as-signal stops being expressible; `PatientStrip` makes the `null` vs `[]` allergy discrimination the API shape, not a per-plate decision.
- **Choice.** (c). The SaaS standard is necessary but not sufficient for clinical UI. The medical bar codified: ambiguity = defect (units always visible adjacent to every value); abnormal must look abnormal (icon + label + color triad — color is *never* the sole signal); stale data is dangerous (timestamps on every clinical data point, persimmon when past freshness threshold); empty ≠ normal (empty states say "not collected" / "no data" / "not documented" — never blank that reads as fine); WCAG AA, keyboard-complete, tap targets ≥ 44px on clinical actions; real LOINC-style lab names, RxNorm-style drug codes, realistic dosing; no celebratory motion, no playful copy, no emojis. Five new clinical primitives in `_kit/` enforce it: `clinical-value.tsx` (atomic value+unit+flag+timestamp), `abnormal-flag.tsx` (severity triad — low/high/critical-low/critical-high/high-alert/panic, icon+letter+color, color never sole signal), `timestamp.tsx` (relative + absolute on hover, UTC-stable, persimmon when stale), `reference-range.tsx` (visual range marker for normal-range context), `patient-strip.tsx` (persistent header that discriminates `null` allergies (`role="alert"`, persimmon, "not documented") from `[]` (NKDA verified)). Refinement pass against the bar across all 13 prior medical plates. Three new medical plates land alongside via head-to-head + 2 rounds of bad-day critique: `clinical/mar` (time-grid MAR — beat row-timeline alternative; nurse's working surface, high-alert flags, missed-dose chip, NOW-column hairline, 2-nurse-verify with witness selector), `dashboards/critical-alerts` (cross-patient unread-abnormal feed — beat master-detail result-inbox alternative; severity by SHAPE not color, 15-min critical-call countdown, J/K nav, ack/escalate/forward), `clinical/problem-list` (longitudinal problem index — status by shape + size for severity, ICD-10, related-meds chip, related-events timeline modal). Bare-preview format fix on three plates (chart-header, care-team-rail, bed-board) — hardcoded ISO timestamps were rendering as "10H AGO" because snap was 10h after the seed time; switched to `format="absolute"` on those banners since relative-staleness vocabulary is demoed elsewhere (vitals-monitor's per-metric stamps).
- **Tradeoff.**
  - **What we lose.** Pure-specimen voice. The medical section now reads more like a working EHR catalog than a printed folio — the gallery has two registers (SaaS plates feel product-grade; medical plates feel product-grade *under a stricter clinical bar*) and the catalogue voice has to carry both. The "every plate is a frozen specimen" auditing-by-eye story is gone for medical too; the audit surface per medical plate is roughly tripled, plus the clinical-correctness axis (units? timestamps? abnormal triad? allergies discrimination?) on top. Future medical plates inherit this bar by default, so the speed-to-first-paint story for new medical work is gone — every new plate ships at THE MEDICAL STANDARD or doesn't ship.
  - **What we gain.** Every medical plate is interview-grade for a clinician portfolio reader. The clinical primitives standardize the vocabulary across the 16 medical plates — a reader who sees `<ClinicalValue>` on `lab-results` recognizes the same atomic unit on `vitals-monitor`, `medication-list`, `mar`, `critical-alerts`. The clinical bar is enforced by API shape, not by reviewer vigilance — `ClinicalValue` doesn't accept a value without a timestamp; `PatientStrip` discriminates allergy state at the type level. Three new plates that wouldn't have existed under the specimen framing (`mar`, `critical-alerts`, `problem-list`) land as the centerpieces of the medical section. Reverses the prior "Medical retains specimen framing" item without contradicting the SaaS specimen-shift decision — the SaaS shift relaxed only specimen *framing*; this entry codifies a *stricter* product-grade bar for the domain that demands it.
  - **The line that holds.** The aesthetic and motion budget are *not* relaxed. The clinical primitives compose without introducing new keyframes, new color tokens beyond the existing persimmon/Federal-Blue/walnut triad, or new motion. Loading skeletons remain opacity-only crossfade (no shimmer). Forbidden patterns from `CLAUDE.md` hold unchanged. Color-only signaling for clinical state is now *additionally* forbidden; abnormal must look abnormal via icon + label + color, never color alone.
- **Signal.** Five new clinical primitives in `components/_kit/`: `clinical-value.tsx`, `abnormal-flag.tsx`, `timestamp.tsx`, `reference-range.tsx`, `patient-strip.tsx`. Refinement pass landed across all 13 prior medical plates against the bar. Three new medical plates: `clinical/mar`, `dashboards/critical-alerts`, `clinical/problem-list` (head-to-head losers `mar-alt` and `result-inbox` culled). Bare-preview format fix on chart-header, care-team-rail, bed-board. 34 e2e tests pass. Audit-split-verify methodology scaled to 5 builders for medical. Catalogue now 13 SaaS / 16 medical = 29 plates; medical leads after the +3.
